...
let speechModule = {
    ...
    Parrot: (ev)=>{
        ...
        let WhatToSay;
        if(ev.target){ WhatToSay = new SpeechSynthesisUtterance(ev.target.textContent) }
        else {
...
var speechRecognitionList = new e.SpeechGrammarList();
    ...
    ContentFill: (target, spell)=>{
        ...
        if(spell[target]){
            ...
            if(Array.isArray(spell[target])){spell[target] = spell[target].join("<br>");
                                            
            console.log(spell[target])}
            targetzone.querySelector(".Mutable").innerHTML = spell[target]}
        
        else{ targetzone.classList.add("hidden")}
    }
}
